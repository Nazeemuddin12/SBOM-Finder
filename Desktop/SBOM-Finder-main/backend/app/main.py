...
def build_item_detail(item):
    ...
    # Include normalized OS entries alongside the legacy field
    os_list = [{"id": os.id, "os_name": os.os_name} for os in item.os_entries]
    ...
    return {
        ...
        "operating_system": item.operating_system,  # legacy
        "os_entries": os_list,
...
# ---------------------------------------------------------------------------
# Item OS endpoints (US-21)
# ---------------------------------------------------------------------------

@app.get("/items/{item_id}/os", response_model=list[ItemOSResponse])
def get_item_os(
    item_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    return item.os_entries


@app.post("/items/{item_id}/os", response_model=ItemOSResponse)
def add_item_os(
    item_id: int,
    payload: ItemOSCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user),
):
    item = db.query(models.Item).filter(
    ...
    # Prevent duplicate OS entries on the same item
    existing = db.query(models.ItemOS).filter(
        models.ItemOS.item_id == item_id,
        models.ItemOS.os_name == payload.os_name,
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"{payload.os_name} already added")

    os_entry = models.ItemOS(item_id=item_id, os_name=payload.os_name)
    db.add(os_entry)
    db.commit()
    db.refresh(os_entry)
    return os_entry
...
@app.delete("/items/{item_id}/os/{os_id}")
def delete_item_os(
    item_id: int,
    os_id: int,
    db: Session = Depends(get_db),
    ...
    os_entry = db.query(models.ItemOS).filter(
        models.ItemOS.id == os_id,
        models.ItemOS.item_id == item_id,
    ).first()
    if not os_entry:
    ...
    db.delete(os_entry)
    db.commit()
    return {"message": f"{os_entry.os_name} removed"}
...
def search_items(
    ...
    if operating_system:
        query = query.filter(
            or_(
                models.Item.operating_system.ilike(f"%{operating_system}%"),
                models.Item.os_entries.any(
                    models.ItemOS.os_name.ilike(f"%{operating_system}%")
                ),
            )
        )
    if name:
...
def copy_public_to_workspace(
    ...
    # Also copy OS entries to the new item
    for os_entry in public_item.os_entries:
        db.add(models.ItemOS(item_id=new_item.id, os_name=os_entry.os_name))
...
async def generate_sbom(
    ...
    if not is_syft_available():
        raise HTTPException(
            status_code=503,
            detail="Syft is not installed on this server yet."
        )
