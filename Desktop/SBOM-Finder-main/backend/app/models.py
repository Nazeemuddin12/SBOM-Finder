...
class Item(Base):
    ...
    operating_system = Column(String, nullable=True)  # legacy field — kept for migration safety
    ...
    os_entries = relationship("ItemOS", back_populates="item", cascade="all, delete-orphan")


class ItemOS(Base):
    # Normalized OS table — replaces the plain-text operating_system field on Item
    # Each row is one platform (e.g. "Windows", "Linux", "macOS")
    # Admins can add new platforms without touching code
    __tablename__ = "item_os"

    id = Column(Integer, primary_key=True, index=True)
    item_id = Column(Integer, ForeignKey("items.id"), nullable=False, index=True)
    os_name = Column(String, nullable=False)   # e.g. "Windows", "Linux", "macOS", "Android"

    item = relationship("Item", back_populates="os_entries")
