  ...
  return (
    <div className="page-shell">
      ...
      <section className="hero">
        ...
        <p>Search for products and applications that include a given library, component, or software material.</p>
      </section>

      <section className="section-card" style={{ marginBottom: "22px" }}>
        <h2 className="section-title">Search Filters</h2>
        <p className="section-subtitle">Use a component name like OpenSSL, zlib, Log4j, or curl.</p>

        <div className="filter-grid">
          <input
            type="text"
            placeholder="Component name (e.g. OpenSSL)"
            value={componentName}
      ...
      {searched && !loading && !error && results.length === 0 && (
        <section className="section-card">
          <div className="empty-state">No matching items found for this component.</div>
        </section>
      )}
      ...
      {results.length > 0 && (
        <section className="section-card">
          ...
          <p className="section-subtitle">Click any result to inspect its full SBOM component list.</p>
          <div className="item-list">
            {results.map((item) => (
              <div key={item.id} className="item-card" onClick={() => navigate(`/item/${item.id}`)}>
                <h3>{item.name}</h3>
                <div className="item-meta">
                  <div className="meta-pill"><span>Type</span><strong>{item.item_type || "N/A"}</strong></div>
                  <div className="meta-pill"><span>Manufacturer</span><strong>{item.manufacturer || "N/A"}</strong></div>
                </div>
              </div>
            ))}
          </div>
